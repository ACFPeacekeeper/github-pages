...
1. Get a reliable repro: the exact command (`npm run dev`/`npm run build`/`npm test`/`npx cypress run`) and the full error output.
...
5. Once fixed, add a regression test (Jest for components/logic, Cypress for a broken user flow) and note the root cause in the commit message.
