...
| Optional self-hosting | `infra/global/` (docker, k8s, helm, terraform, ansible, cloud, wordpress) | per-tool configs (no shared lockfile) | Docker / kubectl / helm / terraform / ansible / cloud CLIs |
| Dev-only infra | `infra/private/` (webpack) | none (sample config) | — |
